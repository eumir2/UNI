...
function getSeries(uri) {
    ...
    fetch(url)
        .then(response => response.json())
        .then(tmp => loadData(tmp.data.results, "contenitoreSerie"))
...
function getEvents(uri) {
    ...
    fetch(url)
        .then(response => response.json())
        .then(tmp => loadData(tmp.data.results, "contenitoreEventi"))
        .catch(error => console.log('error', error))
...
function getComics(uri) {
    ...
    fetch(url)
        .then(response => response.json())
        .then(tmp => loadData(tmp.data.results, "contenitoreComics"))
        .catch(error => console.log('error', error))
...
function loadData(tmp, id) {
    ...
    var slide = contenitore.getElementsByClassName("dato")[0];

    //console.log(slide);

    if(tmp.length == 0){
        contenitore.innerHTML = "NESSUN DATO DISPONIBILE";
        return;
    }
    ...
    for(let i = 0; i < tmp.length; i++){
        ...
        clone.getElementsByClassName("img")[0].src = tmp[i].thumbnail.path + "/portrait_fantastic." + tmp[i].thumbnail.extension;
        clone.getElementsByClassName("titolo")[0].innerHTML = tmp[i].title.substring(0, 60) + "...";
